...
module Make (C : Context) : sig
  ...
  module T : sig
    ...
    val intify : [> zbool ] term -> [> zint ] term
  ...
  module Optimize : sig
    ...
    val maximize : solver:t -> [< znum ] term -> handle
    val minimize : solver:t -> [< znum ] term -> handle
    val get_upper : handle -> (Q.t, [> znum ]) symbol
    val get_lower : handle -> (Q.t, [> znum ]) symbol
  end
