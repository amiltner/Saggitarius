...
module FixHashMemoizerOf (F : UnfixedHashFunction) = struct
  let result_storage : (F.Key.t, F.Result.t) Hashtbl.t =
    Hashtbl.create (module F.Key)
  ;;

  let rec evaluate (x : F.Arg.t) : F.Result.t =
    let key = F.extract_key x in
    match Hashtbl.find result_storage key with
    | None ->
      let y = F.f evaluate x in
      Hashtbl.add_exn ~key ~data:y result_storage;
      y
    | Some y -> y
  ;;
end
