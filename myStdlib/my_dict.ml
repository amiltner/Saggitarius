...
module DictOf (K : Data) (V : Data) = struct
  type key = K.t [@@deriving ord, show, hash, eq]
  type value = V.t [@@deriving ord, show, hash, eq]
  type pair = key * value [@@deriving ord, show, hash, eq]
  ...
  let rec map_values ~(f : value -> value) (d : t) : 'a =
    match d with
    | Leaf -> Leaf
    | Two (left, (k, v), right) ->
      Two (map_values ~f left, (k, f v), map_values ~f right)
    | Three (left, (k1, v1), middle, (k2, v2), right) ->
      Three
        ( map_values ~f left
        , (k1, f v1)
  ...
  let insert_upward_two
    ...
    if is_equal cmp
    then Done (Two (w_left, w, w_right))
    else if is_lt cmp
  ...
  let insert_upward_three
    ...
    match
      ...
      , make_matchable (compare_key w_key y_key) )
    with
  ...
  let rec insert_downward
    ...
    | Two (left, n, right) -> insert_downward_two merge (k, v) n left right
  ...
  and insert_downward_two
    ...
    if is_equal cmp
    then Done (Two (left, (k1, merge v1 v), right))
    else if is_lt cmp
    then begin
  ...
  and insert_downward_three
    ...
    =
    match
      make_matchable (compare_key k k1), make_matchable (compare_key k k2)
    with
  ...
  (* DO NOT EDIT THIS *)
  let rec remove_downward (updater : value -> value option) (d : t) (k : key)
      : hole
    =
    match d with
    | Leaf -> Absorbed (None, d)
    | Two (Leaf, (k1, v1), Leaf) ->
      if is_equal (compare_key k k1)
      then (
        let rem = Some (k1, v1) in
        match updater v1 with
        | None -> Hole (rem, Leaf)
        | Some v1 -> Absorbed (rem, Two (Leaf, (k1, v1), Leaf)))
      else Absorbed (None, d)
    | Three (Leaf, (k1, v1), Leaf, (k2, v2), Leaf) ->
      ...
      | EQ, _ ->
        ...
        begin
          ...
          | Some v1 ->
            Absorbed (rem, Three (Leaf, (k1, v1), Leaf, (k2, v2), Leaf))
        end
      | _, EQ ->
        let rem = Some (k2, v2) in
        begin
          match updater v2 with
  ...
  and remove_downward_three
      ...
      begin
        ...
        | None ->
          begin
            ...
            | Hole (None, _) -> Hole (rem, Two (left, (k2, v2), right))
            | Hole (Some n, new_middle) ->
              remove_upward_three n (k2, v2) rem left new_middle right Mid3
            | Absorbed (None, _) -> Absorbed (rem, Two (left, (k1, v1), right))
    ...
    | GT, LT ->
      begin
        match remove_downward updater middle k with
        | Hole (rem, t) ->
  ...
  let remove_or_update_and_retrieve_old
      ...
      : value option * t
    =
    match remove_downward updater d k with
  ...
  let remove_or_update_and_retrieve_old_exn
      ...
      : value * t
    =
    let vo, t = remove_or_update_and_retrieve_old ~updater d k in
    Option.value_exn vo, t
  ;;

  let remove_or_update ~(updater : value -> value option) (d : t) (k : key) : t =
    snd (remove_or_update_and_retrieve_old ~updater d k)
  ;;

  (* DO NOT EDIT THIS *)
  let remove : t -> key -> t = remove_or_update ~updater:(fun _ -> None)

  let update ~(updater : value -> value) : t -> key -> t =
  ...
  let lookup_default ~(default : value) (d : t) (k : key) : value =
  ...
  let member (d : t) (k : key) : bool = Stdlib.( <> ) (lookup d k) None
  ...
  let from_kvp_list (l : (key * value) list) : t =
    List.fold_left ~f:(fun d (k, v) -> insert d k v) ~init:empty l
  ;;

  let singleton (k : key) (v : value) : t = insert empty k v
  ...
  let as_kvp_list (d : t) : (key * value) list =
    fold (fun k v l -> (k, v) :: l) [] d
  ;;

  let key_list (d : t) : key list = List.map ~f:fst (as_kvp_list d)
  let value_list (d : t) : value list = List.map ~f:snd (as_kvp_list d)

  let compare (d1 : t) (d2 : t) : comparison =
  ...
  let equal (d1 : t) (d2 : t) : bool = comparison_to_equality (compare d1 d2)
  ...
  let merge
    ...
    let rec merge_ordered_lists
      ...
      | (k1, v1) :: t1, (k2, v2) :: t2 ->
        begin
          match make_matchable (compare_key k1 k2) with
  ...
  let merge_to_dict
      ?(only_d1_fn : value -> value = ident)
      ?(only_d2_fn : value -> value = ident)
      ~(combiner : value -> value -> value)
      (d1 : t)
      (d2 : t)
      : t
    =
    from_kvp_list (merge ~combiner ~only_d1_fn ~only_d2_fn d1 d2)
  ;;

  let rec max_key (d : t) : key option =
    match d with
    | Leaf -> None
  ...
  let max_key_exn (d : t) : key = Option.value_exn (max_key d)
  let size : t -> int = fold (fun _ _ acc -> acc + 1) 0

  let first ~(f : key -> value -> bool) : t -> (key * value) option =
    fold
      (fun k v fo ->
        match fo with
  ...
  let first_exn ~(f : key -> value -> bool) (d : t) : key * value =
    Option.value_exn (first ~f d)
  ;;

  let exists ~(f : key -> value -> bool) (d : t) : bool =
    let first_matching_o = first ~f d in
    Option.is_some first_matching_o
  ;;

  let for_all ~(f : key -> value -> bool) : t -> bool =
    not % exists ~f:(not %% f)
  ;;
end
