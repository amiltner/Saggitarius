  ...
  and add t d =
    ...
    let rec loop i =
      ...
      then begin
        ...
        if newsz <= sz
        then failwith "Hashcons.Make: hash bucket cannot grow more";
...
module HashConsOf (D : Data) = struct
  type t = t_node hash_consed

  and t_node = D.t [@@deriving show, hash]
  ...
  let uid (x : t) : int = x.tag
end

module Hmap = struct end
