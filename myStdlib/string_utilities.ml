...
let delimit_tabs : string -> string = Str.global_replace (Str.regexp "\t") "\\t"
...
let string_of_list (inner_converter : 'a -> string) (al : 'a list) : string =
  bracket (String.concat ~sep:";" (List.map ~f:inner_converter al))
;;
...
let string_of_pair
    ...
    : string
  =
  paren (first_converter a ^ "," ^ second_converter b)
;;
...
let string_of_triple
    ...
    : string
  =
  paren (fst_to_string a ^ "," ^ snd_to_string b ^ "," ^ trd_to_string c)
;;
...
let string_of_char_list : char list -> string = string_of_list Char.to_string
...
let to_string_of_printer (printer : 'a -> Format.formatter -> unit) (x : 'a)
    : string
  =
