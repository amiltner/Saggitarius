...
module GrammarSpec = struct
  ...
  (*let nullable_prov
        ...
        (g:t)
      : NullablesDict.t =
      let nm = nts_to_number_map g in
      let rec populate_nullables
          (init:Nts.t)
          (current_dict:NullablesDict.t)
        : NullablesDict.t =
        if Option.is_some (NullablesDict.lookup current_dict init) then
          current_dict
        else
          let outgoing_dict = NullablesDict.insert current_dict init None in
          let all_prods =
            List.map
              ~f:(fun i -> lookup_exn g (NumberedId.create i init))
              (NtsToNumber.lookup_exn nm init)
          in
          let all_children =
            List.dedup_and_sort
              ~compare:Nts.compare
              (List.concat_map
                 ...
                 all_prods)
          in
          let populated_dict = populate_nullables
          failwith "ah"
      in
      populate_nullables
        init
  ...
  let extract_nullable_prov (item : EarleyItem.t) (nullables : NullablesDict.t)
      : ProvRecorder.t
    =
    NullablesDict.lookup nullables item.head.id
  ;;
