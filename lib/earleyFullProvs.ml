...
let create_items (item : EarleyItem.t) grammar startidx nullable ntsmap
  ...
  let p2eis l (nid, prod) : EarleyItem.t list =
    (*let sr = ProvRecorder.SR (nid,old_prov) in *)
    let newitem =
      EarleyItem.create
        ~par:[]
        ~unpar:prod
...
let pred
  ...
  let rec filter_duplicates ni l (d : EarleyDict.t) =
    ...
    | [] -> ni, d
    | buffitem :: tl ->
      (match EarleyDict.get_item currentdict buffitem with
      | Some ei ->
        (*let anotherWay = ProvRecorder.Single buffitem.prov in *)
...
let upgrade (item : EarleyItem.t) (item2 : EarleyItem.t) : EarleyItem.t =
  ...
  | nts :: tl ->
    ...
    (*let pnew = ProvRecorder.DR (item2.prov, item.prov) in (*i think this is a better combine than the other way*)
          let rnew = ProvRecorder.add pnew ProvRecorder.empty in*)
    (*let _ = print_endline ("Upgraded: " ^ (ProvRecorder.show rnew)) in*)
    t_provU := !t_provU +. Sys.time () -. !t3;
    reset t3;
    let first_subcomponent =
      ProvRecorder.and_ (ProvRecorder.rule item.head.idx) item.prov
    in
    let item_element = ProvRecorder.and_ first_subcomponent item2.prov in
    EarleyItem.create
...
let create_starting_dict (grammar : gramspec) nts ntsToNumber : EarleyDict.t =
  ...
  let pairfold (currentdict : EarleyDict.t) (kvp : NumberedId.t * Production.t)
    ...
    =
    let nid, prod = kvp in
    let leaf = ProvRecorder.one in
    let newitem =
      EarleyItem.create ~par:[] ~unpar:prod ~start:0 ~head:nid ~prov:leaf
    in
...
let process_last_set (lastset : EarleySet.t) (nts : Nts.t) : ProvRecorder.t =
  let rec aux (item : EarleyItem.t) : ProvRecorder.t option =
    let sameName = Nts.equal nts item.head.id in
    let startAt0 = item.start = 0 in
    if sameName && startAt0
    then Some (ProvRecorder.and_ (ProvRecorder.rule item.head.idx) item.prov)
    else None
  in
  let result =
    ProvRecorder.multi_or (List.filter_map ~f:aux (EarleySet.as_list lastset))
  in
  result
;;

let process_last_dict (lastdict : EarleyDict.t) (nts : Nts.t) : ProvRecorder.t =
