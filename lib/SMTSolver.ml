...
let solve (sat : [> zbool ] term) (prefs : ProcedPrefs.t list)
  ...
  let _ =
    ...
    | [] -> ()
    | _ ->
      let s = List.map ~f:ProcedPrefs.to_objective prefs in
      let soft = T.add s in
      let _ = Optimize.maximize ~solver soft in
      ()
    (* Add the maximization constraint *)
  in
  let result = Optimize.check ~solver [] in
  match result with
  | Unsat _ -> None
